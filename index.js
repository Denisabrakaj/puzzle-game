...
// we change width or height in order to keep the picture size ratio
    ...
    this.reqWidth = params.width - 6;
    this.height = this.reqHeight - 6 * Puzzle.MARGIN1; // place left on screen including margin
    this.width = this.reqWidth - 6 * Puzzle.MARGIN1; //
...
// divBoard
    ...
    */
    ...
    this.divGame.style.width = this.divBoard.style.width = this.width + 1 * Puzzle.MARGIN1 + "px";
    this.divGame.style.height = this.divBoard.style.height = this.height + 1 * Puzzle.MARGIN1 + "px";
...
// re - evaluation of the dimensions of the picture, leaving a space for pieces on one side
    ...
    if (this.image.width / this.image.height <
        (this.reqWidth - 3 * Puzzle.MARGIN1) / (this.reqHeight - 2 * Puzzle.MARGIN1)) {
        ...
        if (this.width > this.reqWidth - 5 * Puzzle.MARGIN1) {
            this.width = this.reqWidth - 5 * Puzzle.MARGIN1;
            this.height = this.width * this.image.height / this.image.width - 3;
        }
        this.freeSpace = 1; // place left under
    }

    let height = this.height - 3, width = this.width - 3;
...
// but keep the canvMobile
    ...
    this.canvMobile.width = this.reqWidth - 4;
    ...
    for (kp = 0; kp < this.polyPieces.length; kp++) {
        for (kn = 0; kn < this.polyPieces[kp].pieces.length; kn++) {
            ...
            switch(this.freeSpace){
                case 0 : pc.pTarget = new Point(this.reqWidth - (2.75 + Math.random() / 2) * this.dx, Math.random() * (this.height -  this.dy) - this.dy);break;
                case 1 : pc.pTarget = new Point(Math.random() * (this.width -  this.dx) - this.dx, this.reqHeight - (2.75 + Math.random() / 2) * this.dy);
            } // switch
        } // for kn
    } // for kp
    window.setTimeout((function(obj) {return function() {obj.launchAnimation()}})(this), 1);
...
// limit the duration to the range 0.25..2s, i.e.12..100 steps
    ...
    this.anim.tmr = window.setInterval((function(puzz){return function(){puzz.animateEnd()}})(this), 1);
